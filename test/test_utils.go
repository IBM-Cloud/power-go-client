...
func getSession(t *testing.T) *ps.IBMPISession {
	// new version
	if apiKey != "" {
		return getSessionAPIKey(t)
	}
	// deprecated version
	return getSessionBearerToken(t)
}

// getSessionAPIKey(): Uses API key for authentication | Perfered
func getSessionAPIKey(t *testing.T) *ps.IBMPISession {
	// set authentication url for env production | staging
	// default: production
	authenticatonUrl := "https://iam.cloud.ibm.com"
	...
	session, err := ps.NewSessionApiKey(sessionOptions)
	if err != nil {
		t.Fatal(err)
	}
	return session
...
// getSessionBearerToken(): Uses bearer token for authentication | Deprecated (working)
func getSessionBearerToken(t *testing.T) *ps.IBMPISession {
	session, err := ps.New(bearerToken, region, debug, accountID, zone)
	if err != nil {
		t.Fatal(err)
	}
	return session
...
// waitForJobState(): Wait until job is completed to continue test function
func waitForJobState(jobClient *client.IBMPIJobClient, jobId string, timeout int) error {
	var status string
